...
Hi ha múltiples teixits afectats, sobretot els més dependents de la generació d'ATP via mitocondrial (cervell, múscul, òrgans endocrins)...
...
La deleció inclou la zona entre els orígens de replicació del mtDNA. Hi ha qui defensa que les formes delecionades de mtDNA tenen un avantatge sobre les molècules de mtDNA intactes.

Sembla que l'origen està en la generació d'estructures tipus loop quan es replica el mtDNA.

La deleció pot afectar tRNA.

\subsubsection{Malalties per mutació puntual}
\label{sec:malalt-per-mutac}

\begin{itemize}
\item MELAS (\textit{Myiopathy, stroke-like episodies, lactic acidosis})

\item MERRF (\textit{myoclonic epiplepsy}). Es troba \textit{red ragged fibers} a les fibres musculars.

\item  NARB \textit{neuropatia, atàxia i retinitis pigmentosa}: Mutació puntual a ATP6.
\end{itemize}

La síndrome de Lay provoca apoptosi al cervell (encefalopatia necrotitzant). Tenen una mutació a ATP6 i més heteroplàsmia que NARB.

LHON és d'herència materna (abans moltes malalties apareixien \textit{de novo}). Afecta el nervi òptic. Diferents mutacions causals que afecten els gens ND (complex I de la cadena respiratòria) al nervi òptic. La cèl·lula té adaptacions per viure sense NS.
