...
Es treballa normalment amb plasmidis però també es fan servir siRNA o oligonucleòtids.
...
L'estructura bàsica de la molècula és: grup catiònic-espaiador-lípid. El component lipídic pot ser diacilglicerol o colesterol entre d'altres, i és la part hidròfoba que permet l'auto-agregacio? com en la bicapa lipídica de la membrana cel·lular. El grup catiònic, és una amina terciària, polilisina, o altres, i permet la condensació amb els àcids nucleics per interaccions electrostàtiques amb els grups fosfat carregats negativament. Els liposomes catiònics acostumen a incloure lípids neutres (colesterol) que tenen funció coadjuvant d'estabilitzar la micel·la.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{fig55}
\label{fig55}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{fig57}
\caption{Esquerre: estructura multilaminar. Dreta: estructura hexagonal}
\label{fig57}
\end{figure}
...
\paragraph{Captació cel·lular} \hfill \\
...
\paragraph{Aplicacions \textit{in vitro}} \hfill \\
...
\paragraph{Aplicacions \textit{in vivo}} \hfill \\
...
\centering
\includegraphics[width=0.8\textwidth]{fig58}
\caption{Dendrímer: estructura d'abre formada per la repetició de subunitats brancades unides al voltant d'un nucli.}
\label{fig58}
\end{figure}
...
\paragraph{Polications biodegradables} \hfill \\
...
\caption{Descripció dels CPP més utilitzats. Els aminoàcids en negreta tenen càrrega positiva.}
\label{fig61}
\end{figure}
...
\paragraph{Mecanismes de captació} \hfill \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig63}
\caption{a) Formació del complex CPP?àcid nucleic. b) Mecanismes d?internalització (Cav, endocitosi mediada per caveolina; CCV, endocitosi mediada per clatrina; MP, macropinocitosi; Pen, penetració). c) Tràfic intracel·lular dels complexos.}
\label{fig63}
\end{figure}

\paragraph{Aplicació \textit{in vitro}} \hfill \\
...
\paragraph{Aplicació \textit{in vivo}} \hfill \\
...
L'aplicabilitat \textit{in vivo} està limitada pel pes, càrrega elèctrica i immunogenicitat de la molècula. Un risc és la interacció dels vectors amb components de la sang i la inducció de trombes.
...
Un prototip multi-funcional de vector proteic que incorpora diversos dominis, cadascun amb una funció diferent, en una única cadena és el següent:

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig64}
\label{fig64}
\end{figure}

Aquest vector proteic incorpora:
\begin{itemize}
\item Un pèptid fusogènic (FP) per desestabilitzar les membranes dels endosomes i sinèrgicament amb els residus d'histidina pertorbar l?endosoma. ?
\item Domini únic catiònic: RRVRR (on R és arginina i V és valina) per condensar pDNA, i histidina (H) per interrompre els endosomes a través de l'efecte d?esponja de protons. ?
\item Un senyal de localització nuclear (NLS), conegut com M9-NLT, per ajudar en la translocació activa del material genètic cap al nucli cèl·lular. ?
\item Un motiu de reconeixement (TM) per les cèl·lules canceroses que sobreexpressen HER2.
\end{itemize}

10 mg de biopolímer és produït per1 litre de cultiu d'\textit{E. coli}, la qual cosa és suficient per permetre tots els estudis preclínics d'una manera cost-efectiva.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{fig65}
\label{fig65}
\end{figure}
...
Diverses molècules es poden vincular a les NPMs per tal d'alliberar la seva càrrega molecular associada a la cèl·lula a través de la internalització per la via endoci?tica.
...
\paragraph{Micro(nano)bombolles} \hfill \\
...
\paragraph{Micro(nano)partícules biodegradables} \hfill \\
...
\paragraph{Transfecció òptica (optoinjecció)} \hfill \\
...
\paragraph{Ones de xoc induïdes per làser} \hfill \\
El polsos de làser de 532 nm, curts (nanosegons o femtosegons) i d?alta energia, aplicats sobre un material diana absorbent, creen ones de xoc (ones d?estrès) que interaccionen amb la cèl·lula com forces mecàniques i permeabilitzen transitòriament la membrana plasmàtica, i també la membrana nuclear. 

Aquestes ones són un efecte col·lateral nociu dels teixits en els tractaments mèdics amb làser. 
La capacitat de captació de molècules exògenes per les cèl·lules està estretament relacionada amb l'impuls de l?ona de xoc, la distància entre el làser i les cèl·lules i el pes molecular de la molècula exògena. 

Aquesta tècnica permet tractar un gran nombre de cèl·lules simultàniament. 

El làser Q-commutació Nd:YAG pot generar nanosecond-pulsed laser-induced stress wave (LISW), irradiant un objectiu sòlid de material elàstic (com és un disc de cautxú negre (polímer del isoprè). El làser crea plasma (matèria en un estat ionitzat semblant a gas), la ràpida expansió del qual indueix la formació d?ones de xoc compressives. La diana sòlida es cobreix d?un polímer transparent de (polietilè tereftalat) que permet intensificar l?impuls de les ones en confinar el plasma induït pel làser. S?apliquen polsos de làser d?1 ns i 532 nm. 

\paragraph{Aplicabilitat \textit{in vitro}} \hfill \\
Les cèl·lules formen una monocapa a la part inferior d'un pou. El blanc de làser és un disc de cautxú negre sota el qual es troba una capa de polímer transparent per confinar la inducció de plasma. A alta irradiació la major part de l'energia de la llum incident es consumeix en la ionització del blanc i només una petita fracció es converteix en energia tèrmica interna. 

\paragraph{Aplicabilitat \textit{in vivo}} \hfill \\
L?àcid nucleic s?injecta en el teixit d'interès i es situa la diana del làser al teixit.?
Es pot aplicar a teixits externs i també utilitzant catèters a teixits interns. La penetració és de l?ordre de mil·límetres i variable en funció del teixit.

\subsubsection{Microprojectils. Pistoles de DNA}
\label{sec:micr-pist-de}

Es basa en adsorbir el DNA a micropartícules que són propulsades a gran velocitat (430 m/s) sobre les cèl·lules.

En el mètode convencional s?empren micropartícules esfèriques de 4 ?m de tungstè o or.?Les micropartícules recobertes amb el DNA es disposen a l?interior d?una pistola de DNA que les propulsa. 

La força de l?impacte fa que algunes partícules arribin directament al nucli de manera que el DNA es introduït directament i intacte. Les cèl·lules continuen viables excepte que rebin l'impacte de molts microprojectils, llavors es poden observar cristalls. 

L'expressió és principalment temporal, ja que el DNA tendeix a romandre lligat a la partícula i no s?integra, però els gens s?expressen. 

\paragraph{Aplicabilitat \textit{in vivo}} \hfill \\
Aquest mètode s?utilitza principalment in vivo, en teixits superficials com la pell. Es pot usar en teixits interns amb intervenció quirúrgica. 

Una limitació del sistema és l?abast del teixit afectat que es limita al diàmetre de la boca de la pistola. Dona lloc a reaccions transitòries d?inflamació.?L'aplicació més exitosa es la d?induir immunització, amb vacunes basades en DNA. 

\subsubsection{Microinjecció}
\label{sec:microinjeccio}
Es tracta d?introduir el DNA directament al nucli mitjançant punció amb micropipetes.?Les micropipetes de vidre tenen un diàmetre d?entre 0.1-0.5 $\mu$m i es dirigeixen per micromanipuladors sota el microscopi invertit. 

La solució de DNA s?aspira a la micropipeta, aproximadament 10-20 fl, i s'expulsa dins la cèl·lula. El volum injectat representa entre l?1-2\% del volum cel·lular i conté unes 100-200 molècules de DNA de mida mitjana (5-8 Kb). 

Es poden injectar fins a 500-1000 cèl·lules per hora i l'eficiència del mètode és del 50-100\%. Una fracció del DNA acostuma a integrar-se, aproximadament el 25\% del que ha estat injectat, de manera que el nombre de copies integrades és proporcional a les injectades encara que es dona un fenomen de saturació. La incorporació es dona en tàndems (cap i cua), en llocs a l'atzar, però en un o pocs llocs dels cromosomes.

\paragraph{Aplicabilitat \textit{in vitro}} \hfill \\
Donat el limitat nombre de cèl·lules que es poden injectar per unitat de temps 
s?aplica bàsicament \textit{in vitro}.

Principalment, s?aplica a oòcits fecundats en els que s?acostuma a injectar el pronucli masculí, amb la consegüent generació d?animals transgènics. En aquest cas, la principal limitació es poder visualitzar al microscopi els pronuclis del zigot, els quals es reconeixen fàcilment en algunes espècies com el ratolí però són poc definits en d?altres com rata o porc. 
