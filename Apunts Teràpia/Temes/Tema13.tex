...
\paragraph{Endonucleases sèriques} \hfill \\
...
\paragraph{Agregació amb proteïnes sèriques} \hfill \\
...
\paragraph{Agregació amb cèl·lules sanguínies i fagocitosi} \hfill \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{fig66}
\label{fig66}
\end{figure}

\paragraph{Excreció renal} \hfill \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{fig67}
\label{fig67}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig68}
\label{fig68}
\end{figure}
...
\item Liposomes: estructures hexagonals invertides.
\end{itemize}
...
\paragraph{Endocitosi mitjançada per receptor} \hfill \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig69}
\label{fig69}
\end{figure}

\paragraph{Fagocitosi} \hfill \\
...
\paragraph{Macropinocitosi} \hfill \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig70}
\label{fig70}
\end{figure}
...
No obstant, en general s'accepta que els CPPs usen sobretot les vies endocítiques per entrar en les cèl·lules. El mètode de captació endoci?tica varia segons la grandària i la naturalesa del complex COP-àcid nucleic. Poden ser vies dependents de clatrina, vies clatrina-independents i macropinocitosi.

\subsection{Sortida de l'endosoma i estabilitat al citoplasma}
\label{sec:sortida-de-lendosoma}

\subsubsection{Alliberament de les vesícules endocítiques}
\label{sec:alliberament-de-les}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig72}
\label{fig72}
\end{figure}
...
\subsubsection{Mobilitat i estabilitat dels àcids nucleics lliures en el citoplasma}
\label{sec:mobil-i-estab}

Els àcids nucleics de cadena simple o doble són degradats al citoplasma per nucleases. Els de cadena simple són més fàcilment degradats que els de cadena doble.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig73}
\caption{Divisió mitòtica de les macromolècules lliures com dextrans, virus i DNA plasmídic.}
\label{fig73}
\end{figure}

\subsubsection{Importació nuclear}
...
L'associació del DNA de doble hèlix amb NLS-proteïnes es pot fer amb diferents estratègies:
\begin{enumerate}[label=\itembolasverdes{\arabic*}]
\item \textbf{El DNA pot incloure seqüències de direccionament al nucli.} Aquestes seqüències interactuen amb les proteïnes (factors de transcripció) que contenen dominis funcional i espaialment diferents d'unió al DNA (DBD) i de translocació nuclear (NLS), de manera que poden interactuar de forma simultània amb el DNA i importines. Aquests factors de transcripció vehiculen l?import del DNA al nucli. No tots els factors de transcripció poden mitjançar la importació del DNA al nucli. 
  \begin{itemize}
  \item Aplicabilitat general: un exemple és el potenciador repetit de 72 pb del genoma del SV40, el qual conté llocs d'unió per diversos factors de transcripció (com AP1-3, NF-kB i Oct-1).
  \item Específics de cèl·lula: com el promotor de l?actina gamma de múscul llis que actua en cèl·lules del múscul llis. 
  \end{itemize}

Aquesta via pot ser manipulada:?es poden formar complexes del DNA amb proteïnes que contenen NLSs in vitro abans del lliurament a les cèl·lules.
...
\item \textbf{El DNA pot lligar-se a un receptor en la superficie cel.lular que dirigeixi la captació cel.lular i la importació nuclear.} La nucleolina pot complir aquestes funcions.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig75}
\caption{Un model de la captació cel·lular de nanopartícules de DNA amb polilisina pegilada (DNPs).?DNPs s'uneixen a la superfície cel·lular a la nucleolina en els lípids rafts, i són internalitzades per endocitosi. Les DNPs unides a nucleolina transiten a través del citoplasma en associació amb el receptor de glucocorticoides (GCR) i complexos de dineína al llarg dels microtúbuls. La nucleolina s'uneix al domini d?unió al DNA del GCR, i permet la seva captació nuclear a través del CPN i les DNPs s'acumulen en nuclèol amb la nucleolina. MTOC, centre d'organització de microtúbuls.}
\label{fig75}
\end{figure}

\item \textbf{Proteïnes de la càpsida viral que dirigeixen la importació nuclear: lentivirus i virus adeno-associats.} Les proteïnes de les càpsides dels lentivirus interactuen amb la maquinària d'importació del CPN. El complex de preintegració de l?HIV-1es genera en el citoplasma, després de la transcripció reversa, té un diàmetre de 56 nm i implica la proteïna de la matriu (MA) i la proteïna viral R (VPR). 

\item \textbf{Càpsides virals acoblables al CPN:} les càpsides dels adenovirus o virus herpes interactuen amb el CPN,  desmantellen la càpsida i alliberen el DNA en el nucleoplasma. DOCKING-INJECTION 

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig76}
\label{fig76}
\end{figure}
...
\subsubsection{Exportació nuclear}
\label{sec:exportacio-nuclear}

Els diferents tipus de RNA que es produeixen al nucli s'exporten a través dels porus nuclears. Els RNA petits com els tRNA o els microRNA segueixen unes rutes d'exportació relativament simples unint-se directament a exportines.

Els RNA més grans s'assemblen en partícules de ribonucleoproteïnes i recluten els exportadors.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{fig77}
\caption{Els adaptadors d'exportació es mostren en blau, els receptors d'exportació es mostren en groc. Els motius estructurals prominents in els preRNA (ssRNA, dsRNA, loops, exons i introns, 5' cap i cua de poliA). Per la ruta d'exportació de mRNA, s'indiquen els noms de les dues proteïnes de metazous i de llevat, i es mostren els mRNAs amb proteïnes adaptadores addicionals i factors d'unió a RNA (ovals taronja). En el cas de rRNA es representen l'exportador general en eucariotes, CRM1, i dos exportadors auxiliars, Mex67-MTR2 i ARX1, que només s'han estudiat en el llevat. CBC, complex d'unió cap; Exp, exportin.}
\label{fig77}
\end{figure}